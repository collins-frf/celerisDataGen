function spectrum_FRF_2D_interp(freq_specvar,theta,E_D,Hs_o,Tp_o,n_cutoff)
...
min_theta=20; % min allowable theta
max_theta=160; % max allowable theta
theta;
Hmo = 0;
nf=length(freq_specvar);
for i=1:nf
   for j=1:length(theta)
      if i==1
         del_f=(freq_specvar(2)-freq_specvar(1));
      elseif i==length(freq_specvar)
         del_f=(freq_specvar(length(freq_specvar))-freq_specvar(length(freq_specvar)-1));
      else
         del_f=(freq_specvar(i+1)-freq_specvar(i))/2.+(freq_specvar(i)-freq_specvar(i-1))/2.;
      end
...
f_max=1/min_period;
f_max_ind=find(freq_specvar>f_max,1);
t_min_ind=find(theta>min_theta,1);
t_min_ind;
t_max_ind=find(theta>max_theta,1);
t_max_ind=(length(theta));
% actually truncate 
if isempty(f_max_ind)
    f_max_ind = length(freq_specvar);
end
freq_specvar=freq_specvar(1:f_max_ind);
length(theta);
...
f_new=[freq_specvar(1):df_new:f_max];
...
E_D=interp2(freq_specvar,theta,E_D',f_new,theta_new','spline');
E_D=E_D';
freq_specvar=f_new;
nf=length(freq_specvar);
theta=theta_new;
...
for i=1:nf
    if i==1
        del_f=(freq_specvar(2)-freq_specvar(1));
    elseif i==nf
        del_f=(freq_specvar(nf)-freq_specvar(nf-1));
    else
        del_f=(freq_specvar(i+1)-freq_specvar(i))/2.+(freq_specvar(i)-freq_specvar(i-1))/2.;
    end
    ...
    for j=1:length(theta)
        ...
        if E_D(i,j)>=E_cutoff
            ...
            file_data(cur_ind,2)=1/(freq_specvar(i));
